#
device=cuda:2
corpus=jvnv
dataset="anger disgust fear happy sad surprise"

#
espnet_csjcore=true
espnet_csjfull=true
espnet_corpus10=true

espnet_stream_corpus10=true
espnet_streamdetrep_corpus10=true

#
sasrct_corpus10=true
sasrct_bb_corpus10=true

#
funasr=true
espnet_laborotv=true
whisperv3=true
reazon=true
nue=false

#
espnet_csjfullcon=true #false

#
kotobaw=true

#
kushinada=true

#
funasrnano=true #false

qwen_17b=true
qwen_06b=true

#
granite412b=true


#
kwsct=true


#########
if "${espnet_csjcore}"; then
    modelname=ouktlab/espnet_csjcore_asr_train_asr_transformer_lm_rnn
    bash ../scripts/run_espnet.sh ${corpus} ${device} ${modelname} 20 0.30 0.21 0.0 ${dataset}
fi

if "${espnet_csjfull}"; then
    modelname=ouktlab/espnet_csj_asr_train_asr_transformer_lm_rnn
    bash ../scripts/run_espnet.sh ${corpus} ${device} ${modelname} 20 0.30 0.21 0.0 ${dataset}
fi

if "${espnet_corpus10}"; then
    modelname=ouktlab/espnet_asr-ja-mc_am-transformer-robustcorpus10_lm-transformer-corpus10-bccwj-wiki40b
    bash ../scripts/run_espnet.sh ${corpus} ${device} ${modelname} 20 0.35 0.195 0.0 ${dataset}
fi

if "${espnet_stream_corpus10}"; then
    modelname=ouktlab/espnet_asr-ja-mc-stream_am-transformer-robustcorpus10_lm-transformer-corpus10-bccwj-wiki40b
    bash ../scripts/run_espnet_stream.sh ${corpus} ${device} ${modelname} 20 0.35 0.195 0.0 ${dataset}
fi

if "${espnet_streamdetrep_corpus10}"; then
    modelname=ouktlab/espnet_asr-ja-mc-stream_am-transformer-robustcorpus10_lm-transformer-corpus10-bccwj-wiki40b
    bash ../scripts/run_espnet_stream_detrep.sh ${corpus} ${device} ${modelname} 20 0.35 0.195 0.0 ${dataset}
fi

##########
if "${sasrct_corpus10}"; then
    sasr_model=ouktlab/espnet_asr-ja-kc_am-transformer-robustcorpus10_lm-transformer-corpus10-bccwj-wiki40b
    tokenizer=ouktlab/character_tokenizer_jis_v2
    sct_model=ouktlab/t5_sct-jis-v2_corpus10-bccwj-wiki40b_mask-1.00
    bash ../scripts/run_sasrct.sh ${corpus} ${device} ${sasr_model} ${tokenizer} ${sct_model} 20 0.35 0.195 0.0 15 ${dataset}
fi

if "${sasrct_bb_corpus10}"; then
    sasr_model=ouktlab/espnet_asr-ja-kc_am-transformer-robustcorpus10_lm-transformer-corpus10-bccwj-wiki40b
    tokenizer=ouktlab/character_tokenizer_jis_v2
    sct_model=ouktlab/t5_sct-jis-v2_corpus10-bccwj-wiki40b_mask-1.00
    bash ../scripts/run_sasrct_bb.sh ${corpus} ${device} ${sasr_model} ${tokenizer} ${sct_model} 20 0.35 0.195 0.0 15 ${dataset}
fi

if "${kwsct}"; then
    kw_model=sbintuitions/kana-whisper
    tokenizer=ouktlab/character_tokenizer_jis_v2
    sct_model=ouktlab/t5_sct-jis-v2_corpus10-bccwj-wiki40b_mask-1.00
    bash ../scripts/run_kwsct.sh ${corpus} ${device} ${kw_model} ${tokenizer} ${sct_model} ${dataset}
fi


##########
if "${espnet_laborotv}"; then
    modelname="Shinji Watanabe/laborotv_asr_train_asr_conformer2_latest33_raw_char_sp_valid.acc.ave"
    bash ../scripts/run_espnet.sh ${corpus} ${device} "${modelname}" 20 0.30 0.21 0.0 ${dataset}
fi

##########
if "${funasr}"; then
    bash ../scripts/run_funasr.sh ${corpus} ${device} ${dataset}
fi

##########
if "${reazon}"; then
    bash ../scripts/run_reazon.sh ${corpus} ${device} ${dataset}
fi

##########
if "${whisperv3}"; then
    bash ../scripts/run_whisperv3.sh ${corpus} ${device} ${dataset}
fi

##########
if "${nue}"; then
    bash ../scripts/run_nue.sh ${corpus} ${device} ${dataset}
fi

##########
if "${espnet_csjfullcon}"; then
    modelname=ouktlab/espnet_csj_asr_train_asr_conformer_lm_rnn
    bash ../scripts/run_espnet.sh ${corpus} ${device} ${modelname} 20 0.30 0.21 0.0 ${dataset}
fi

##########
if "${kotobaw}"; then
    bash ../scripts/run_kotobaw.sh ${corpus} ${device} ${dataset}
fi

##########
if "${kushinada}"; then
    modelname=imprt/kushinada-hubert-large-laborotv2-asr
    bash ../scripts/run_kushinada.sh ${corpus} ${device} ${modelname} 20 0.30 0.21 0.0 ${dataset}
fi

##########
if "${funasrnano}"; then
    bash ../scripts/run_funasrnano.sh ${corpus} ${device} ${dataset}
fi

##########
if "${qwen_17b}"; then
    modelname="Qwen/Qwen3-ASR-1.7B"
    bash ../scripts/run_qwenasr.sh ${corpus} ${device} ${modelname} ${dataset}
fi

if "${qwen_06b}"; then
    modelname="Qwen/Qwen3-ASR-0.6B"
    bash ../scripts/run_qwenasr.sh ${corpus} ${device} ${modelname} ${dataset}
fi

#########
if "${granite412b}"; then
    modelname=granite-speech-4.1-2b
    bash ../scripts/run_granite.sh ${corpus} ${device} ${modelname} ${dataset}
fi

