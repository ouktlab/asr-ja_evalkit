...
#####
#
def proclist(sasr_model, tokenizer, sct_model, filelist, outfile, sasrfile, basedir):
    ...
    with open(filelist) as fin, open(outfile, 'w') as fout, open(sasrfile, 'w') as fsasrout:
        for line in fin:
            ...
            for filepath in filepaths.split(','):
                wavpath = f'{basedir}/{filepath}'
            
                print(f'[LOG]: recognizing -- {key}')
                sasr_results = sasr_model(wavpath)
                sasr_text = sasr_results["text"].replace('、', '')
                sasr_result_text += sasr_text

                input_text = [sasr_text]
...
def load_offline_model(args):
    ...
    model.eval()
    processor = AutoProcessor.from_pretrained(args.sasr_model)
    pipe = pipeline(
    ...
    return pipe, tokenizer, sct_model
...
def main():
    ...
    proclist(sasr_model, tokenizer, sct_model, args.filelist, args.outfile, args.sasrfile, args.basedir)    
