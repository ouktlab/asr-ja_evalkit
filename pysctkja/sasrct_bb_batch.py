    ...
    with open(filelist) as fin, open(outfile, 'w') as fout, open(sasrfile, 'w') as fsasrout:
        for line in fin:
            ...
            for filepath in filepaths.split(','):
                ...
                sasr_texts = []
                sasr_scores = []
                for text, token, token_int, hyp in sasr_results:
                    sasr_texts.append(text)
                    sasr_scores.append(hyp[1].item())        
                sasr_result_text += sasr_texts[0]

                input_text = sasr_texts
                inputs = tokenizer(input_text, return_tensors="pt", padding=True)
                inputs.to(device)
            
                sct_outputs = sct_model.generate(input_ids=inputs.input_ids,
                                             ...
                                             output_scores=True)
                sct_scores = sct_outputs.sequences_scores

                joint_results = []
                for i, output_ids in enumerate(sct_outputs['sequences']):
                    sct_text = tokenizer.decode(output_ids, skip_special_tokens=True)
                    joint_results.append([sct_text, sasr_scores[i] + sct_scores[i]])
                
                joint_results = sorted(joint_results, key=lambda x: x[1], reverse=True)                
                sct_result_text += joint_results[0][0]                
