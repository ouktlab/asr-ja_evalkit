...
# 
if [ $# -le 7 ]; then
    resultpath=espnet/"${modelname}"_detrep/nbeam-${nbeam}_ctcw-${ctcw}_lmw-${lmw}_penalty-${penalty}/
    mkdir -p "${resultpath}"
	...
	python3 ${topdir}/pysctkja/espnet_stream_detrep.py "${modelname}" ${listfile} "${resultfile}" \
    ...
    for listname in ${@:8}; do        
        resultpath=espnet/"${modelname}"_detrep/nbeam-${nbeam}_ctcw-${ctcw}_lmw-${lmw}_penalty-${penalty}/set-${listname}/
        mkdir -p "${resultpath}"
